...
% Para dar continuidade no jogo quando nao ha vitoria nem derrota
% imediata
%proximo_movimento(Jogador, Tabuleiro, ProximaJogada, Resultado) :-
 %   between(0, 8, ProximaJogada), % Iterar sobre as posições do tabuleiro
  %  jogada_valida(ProximaJogada, Tabuleiro),
  ...
  % melhor_continuar(Jogador, NovoTabuleiroModificado),
   %Resultado is 0,
    ...
    %Parar se encontrar uma boa jogada
   % !.
% Para dar continuidade no jogo quando não há vitória nem derrota imediata
proximo_movimento(Jogador, Tabuleiro, ProximaJogada, Resultado) :-
...
continua(Jogador, Tabuleiro, R) :-
    possivel_vitoria(Jogador, Tabuleiro),
    R is 3.
...
possivel_vitoria(Jogador, Tabuleiro) :-
    melhor_jogada(Jogador, Tabuleiro, MelhorJogada),
    copy_term(Tabuleiro, NovoTabuleiro),
    substituir_elemento(NovoTabuleiro, MelhorJogada, Jogador, NovoTabuleiroModificado),
    (   jogo_terminou_vitoria(Jogador, NovoTabuleiroModificado) ->
        true  % Encontrou uma vitória, encerra a recursão
    ;   Tabuleiro \= NovoTabuleiroModificado,  % Se houve mudança no tabuleiro
        possivel_vitoria(Jogador, NovoTabuleiroModificado)  % Continua a busca
    ).
